...
const Node: React.FC<NodeProps> = ({ 
  ...
  // This effect ensures that after any re-render (like an edit),
  // the DOM element's transform is explicitly synced with the React state.
  // This prevents a "jump" that can occur if the imperative drag logic
  // and React's declarative rendering get out of sync.
  useLayoutEffect(() => {
    if (nodeRef.current) {
  ...
  useEffect(() => {
    ...
    const handleMouseDown = (e: MouseEvent) => {
      // Ignore right-clicks, clicks on interactive elements, or multi-select clicks
      if (e.button !== 0 || (e.target instanceof Element && e.target.closest('.nodrag-area, input, button, a')) || e.ctrlKey || e.metaKey) {
        return;
      }
      ...
      // We store the initial position from the person prop when the drag starts
      ...
      const handleMouseMove = (e: MouseEvent) => {
        ...
        // For performance, we update the transform directly on the DOM element
      ...
      const handleMouseUp = (e: MouseEvent) => {
        ...
        // On mouse up, we send the calculated delta to the parent.
        // The parent will update the state, and React will re-render the node
        // in its new final position, overriding our direct DOM manipulation.
        if (dx !== 0 || dy !== 0) {
  ...
  const handleDrop = (e: React.DragEvent) => {
    ...
    const draggedId = e.dataTransfer.getData('application/org-chart-node');
    if (draggedId && draggedId !== person.id) {
        onMoveNode(draggedId, person.id);
    }
  };
  ...
  return (
    <div 
      ...
      style={{
        left: 0,
        top: 0,
        transform: `translate(${person.x || 0}px, ${person.y || 0}px)`,
      }}
    >
      <div
        onClick={(e) => {
            e.stopPropagation();
            if ((e.ctrlKey || e.metaKey) && canEditStructure) {
